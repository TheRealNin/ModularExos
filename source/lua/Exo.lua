...
function Exo:OnCreate()
    ...
    self.thrusterMode = kExoThrusterMode.Horizontal
...
local function ShowHUD(self, show)
    ...
    if self.marineHudVisible == nil or self.marineHudVisible ~= show then
        self.marineHudVisible = show
        ClientUI.SetScriptVisibility("Hud/Marine/GUIMarineHUD", "ExoHud", show)
    end
    
    if self.exoHudVisible == nil or self.exoHudVisible ~= show then
        self.exoHudVisible = show
        ClientUI.SetScriptVisibility("Hud/Marine/GUIExoHUD", "ExoHud", show)
    end
...
function Exo:HandleThrusterStart(thrusterMode)
    ...
    if thrusterMode == kExoThrusterMode.Horizontal then
...
function Exo:ModifyGravityForce(gravityTable)
    ...
    if self:GetIsOnGround() or ( self.thrustersActive and self.thrusterMode == kExoThrusterMode.Horizontal ) then
